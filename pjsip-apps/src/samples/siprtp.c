...
{
    call *call1 = (call *)(entry->user_data);

    PJ_UNUSED_ARG(timer_heap);

    entry->id = 0;
    hangup_call(call1->index);
}
...
static void call_on_state_changed( pjsip_inv_session *inv, 
				   pjsip_event *e)
{
    call *call1 = (call *)inv->mod_data[mod_siprtp.id];
	...
	if (call1->d_timer.id != 0) {
	    pjsip_endpt_cancel_timer(app.sip_endpt, &call1->d_timer);
	    call1->d_timer.id = 0;
	}
	...
	if (app.call_report) {
	    PJ_LOG(3,(THIS_FILE, "Call #%d statistics:", call1->index));
	    print_call(call1->index);
	}
	...
	destroy_call_media(call1->index);

	call1->start_time = null_time;
	call1->response_time = null_time;
	call1->connect_time = null_time;
	...
	pj_gettimeofday(&call1->connect_time);
	if (call1->response_time.sec == 0)
	    call1->response_time = call1->connect_time;

	t = call1->connect_time;
	PJ_TIME_VAL_SUB(t, call1->start_time);

	PJ_LOG(3,(THIS_FILE, "Call #%d connected in %d ms", call1->index,
		  PJ_TIME_VAL_MSEC(t)));

	if (app.duration != 0) {
	    call1->d_timer.id = 1;
	    call1->d_timer.user_data = call1;
	    call1->d_timer.cb = &timer_disconnect_call;

	    t.sec = app.duration;
	    t.msec = 0;

	    pjsip_endpt_schedule_timer(app.sip_endpt, &call1->d_timer, &t);
	}
	...
	if (call1->response_time.sec == 0)
	    pj_gettimeofday(&call1->response_time);
...
static int media_thread(void *arg)
{
    ...
    struct media_stream *strm = (media_stream *)arg;
    char packet[1500];
...
int revAndSend(int recLocalPort, pj_sockaddr rem_addr1);
				  ...
				  pj_status_t status)
{
    ...
    call1 = (call *)inv->mod_data[mod_siprtp.id];
    audio = &call1->media[0];
...
{
	pj_caching_pool cp;
	pjmedia_endpt *med_endpt = app.med_endpt;
	pj_pool_t *pool;
	...
	*/
	...
	status = create_stream(pool, med_endpt, codec_info, dir, recLocalPort,
	...
	/* Create stream based on program arguments */
	status = create_stream(pool, med_endpt, codec_info, PJMEDIA_DIR_ENCODING, recLocalPort + 2,
		&remote_addr, mcast, &mcast_addr,
#if defined(PJMEDIA_HAS_SRTP) && (PJMEDIA_HAS_SRTP != 0)
		use_srtp, &srtp_crypto_suite,
		&srtp_tx_key, &srtp_rx_key,
